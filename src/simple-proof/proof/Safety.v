...
Inductive sta_trm_typ : sta * trm * call_stack -> typ -> Prop :=
| sta_trm_typ_c : forall G s l t T,
    inert G ->
    well_typed G s ->
    G ⊢ t : T ->
    sta_trm_typ (s, t, l) T.
...
Ltac invert_red :=
  match goal with
  | [Hr: (_, _, _) |->l (_, _, _) |- _] => inversions Hr
  end.
...
(** [s: G]                  #<br>#
    ...
    [s': G, G']             #<br>#
    [G, G' ⊢ t': T]         *)
Lemma preservation_helper: forall G s t l s' t' l' T,
    well_typed_l G s ->
    inert G ->
    (s, t, l) |->l (s', t', l') ->
    G ⊢ t : T ->
...
Proof.
  ...
  - Case "thunk_replace".
    ...
    unfold well_typed_l in *.
    destruct_all.
    repeat split; try solve [simpl_dom; auto].
    + eauto using ok_middle_change.
    + intros.
...
(** [⊢ (s, t): T]           #<br>#
    ...
    or [exists s', t'] such that [(s, t) |->l (s', t')] *)
Theorem progress: forall s t T,
    ⊢ (s, t) : T ->
    norm_form t \/ exists s' t', (s, t) |->l (s', t').
Proof.

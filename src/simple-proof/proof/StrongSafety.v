...
Require Import Binding StrongType Definitions GeneralToTight InvertibleTyping Narrowing
...
Proof.
  ...
  - Case "ty_all_intro".
    ...
    + assert (dom G = dom s) by apply Hwf.
      ...
      apply (strong_typed_push_fun Hwf H3 H1).
  - Case "ty_all_elim".
    invert_red; try trm_val_contra.
    destruct (strong_canonical_forms_fun Hin Hwf Ht1) as [?L [?T [?t [?Bis [?Hsub ?Hty]]]]].
    binds_eq.
  ...
  - Case "ty_new_intro".
    ...
    invert_red.
      match goal with
        | [Hn: ?x # ?s |- _] =>
          pose proof (strong_typed_notin_dom Hwf Hn) as Hng
      end.
      ...
      + apply~ strong_typed_push_precise.
      + assert (G & x ~ V ⊢ trm_var (avar_f x) : V) by auto.
         eapply ty_sub. apply H1. apply* weaken_subtyp.
  - Case "ty_new_elim".
    pose proof (strong_canonical_forms_obj Hin Hwf Ht) as [S [ds [t [Bis [Has Ty]]]]].
...
Proof.
  ...
  - Case "ty_all_elim".
    pose proof (strong_canonical_forms_fun Hi Hwt HT1). destruct_all. right*.
  - Case "trm_new".
  ...
  - Case "ty_new_elim".
    pose proof (strong_canonical_forms_obj Hi Hwt HT). destruct_all. right*.
  - Case "ty_let".
